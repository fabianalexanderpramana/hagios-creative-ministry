        ...
        <form action="{{ route('jadwals.update', $jadwal->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="id_tim" id="id_tim"
                    ...
                    @foreach($tims as $tim)
                        @if($tim->id_ibadah == $jadwal->id_ibadah)
                            <option value="{{ $tim->id }}" {{ $jadwal->id_tim == $tim->id ? 'selected' : '' }}>
                                {{ $tim->nama_tim }}
                            </option>
                        @endif
                    @endforeach
        ...
        fetch(`/dropdown-tim/${ibadahId}`)
            ...
            .then(data => {
                ...
                data.forEach(tim => {
                    ...
                    // Preserve selected tim if it exists
                    const currentTimId = '{{ $jadwal->id_tim ?? "" }}';
                    if (tim.id == currentTimId) {
